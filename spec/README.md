...
- `pilot-evidence-local-append.schema.json`: ignored-local append plan for moderator-approved sanitized pilot summaries.
- `pilot-evidence-local-readback.schema.json`: read-only aggregate over ignored local pilot evidence rows.
...
- `pilot-session-brief.schema.json`: read-only moderator brief joining the setup-comprehension task, generic agent guidance, non-ledger-ready summary draft status, and local evidence command loop.
- `pilot-summary-intake.schema.json`: checked sanitized pilot summary intake classifier before ledger review.
- `pilot-summary-submission.schema.json`: caller-supplied sanitized pilot summary input shape for read-only intake classification.
- `pilot-summary-intake-result.schema.json`: read-only classification result for a caller-supplied sanitized pilot summary.
- `pilot-summary-review.schema.json`: read-only joined review over one sanitized summary's intake classification and local append eligibility.
- `pilot-summary-template.schema.json`: read-only sanitized summary draft template that stays non-ledger-ready until an operator fills and reviews it.
...
- `pilot-supervision-status.schema.json`: read-only supervised pilot operator status for next task, remaining real-session counts, local evidence loop commands, and claim boundaries.
...
- `setup-engine.schema.json`: checked domain-agnostic setup-engine front door with candidate forecast contracts, request readiness, source roles, baseline guidance, forecast-card preview shape, host-wrapper shape, examples, interface bindings, and claim boundaries.
- `setup-engine-request.schema.json`: checked structured setup-engine request input with goal, decision context, outcome, horizon, source hints, resolution hints, baseline hints, and execution-boundary safety flags.
- `prediction-goal-catalog.schema.json`: checked generic host-goal catalog for setup-engine examples across forecastable, needs-clarification, blocked, and rejected cases.
- `agent-guidance.schema.json`: checked agent guidance loop for prompt classification, domain-agnostic clarification questions, required source roles, safe next commands, Helsinki narrowing as an example, and agent instruction rules.
...
- `agent-guidance.md`: checked guidance loop that helps calling agents turn messy developer prompts into safe OPE next moves, starting with reusable setup questions before example-specific narrowing.
- `pilot-evidence-ledger.md`: checked sanitized pilot evidence intake ledger and ignored-local append/readback path.
- `pilot-session-packet.md`: checked real pilot-session packet and sanitization boundary.
- `pilot-session-brief.md`: read-only joined moderator brief for one supervised setup-comprehension session.
- `pilot-summary-intake.md`: checked sanitized pilot summary intake classifier and caller-supplied summary-file classification.
- `pilot-summary-review.md`: read-only joined post-session summary review and append-eligibility boundary.
- `pilot-summary-template.md`: checked sanitized summary draft template and operator command loop.
- `pilot-findings.md`: checked pilot findings readback for real-session evidence and current adoption blockers.
- `pilot-supervision-status.md`: read-only supervised pilot operator status and local evidence command loop.
- `simulated-agent-pilot.md`: checked simulated agent pilot readback for one user prompt, seven generated prompts, and three non-Helsinki setup-comprehension prompts, counted separately from real pilot evidence.
...
- `setup-engine.md`: checked domain-agnostic setup-engine front door and structured request path for agents adding prediction to any host project.
- `prediction-goal-catalog.md`: checked generic prediction goal catalog projected through setup-engine examples.
...
The agent guidance generator writes a checked guidance loop under `spec/fixtures/generated/agent-guidance/`, covering accepted, clarification, blocked, rejected, and response-too-large cases plus a domain-agnostic setup flow, Helsinki narrowing example, and instruction pack.
The pilot evidence ledger generator writes checked sanitized intake examples under `spec/fixtures/generated/pilot-evidence/`, blocking raw transcripts and private data while keeping accepted real-session evidence at zero until actual sanitized pilots are recorded. `pilot-evidence --input-summary <summary.json>` prints a dry-run ignored-local append plan, and `--write-local` appends accepted sanitized summaries to `.ope/live/pilot-evidence/pilot-evidence-ledger.json` only after moderator approval. `lifecycle-operation-store --scenario pilot-evidence-append` binds that explicit local write to an operation receipt, idempotency key, lease, `pilot_evidence_ledger_row` payload hash, and `pilot_findings` read model without updating calibration or track-record read models.
...
The pilot session brief generator writes a checked joined moderator brief under `spec/fixtures/generated/pilot-session-brief/`, binding the setup-comprehension task, generic agent guidance, non-ledger-ready summary draft status, and explicit local evidence commands without running sessions or writing evidence.
The pilot summary intake generator writes checked classification examples under `spec/fixtures/generated/pilot-summary-intake/`, marking sanitized summaries as ledger-ready, redaction-needed, or blocked without writing ledger rows or counting real sessions. The same command can classify caller-supplied sanitized summary files with `--input`; checked sample submissions live under `spec/fixtures/pilot-summary-intake/`.
The pilot summary review generator writes a checked joined review under `spec/fixtures/generated/pilot-summary-review/`, binding one sanitized summary file to intake classification, dry-run append eligibility, and explicit local evidence next actions without writing evidence.
The pilot summary template generator writes a checked draft template under `spec/fixtures/generated/pilot-summary-template/`, providing a schema-valid but non-ledger-ready summary draft, field guidance, sanitization checklist, and local command sequence for operators.
The pilot supervision status generator writes a checked operator readback under `spec/fixtures/generated/pilot-supervision-status/`, recommending the setup-comprehension task, reporting remaining real-session thresholds, and showing the exact classify, explicit local append, findings, and status-review commands without writing local evidence itself.
The simulated agent pilot generator writes a checked user-authorized simulation under `spec/fixtures/generated/simulated-agent-pilot/`, covering one user prompt and seven generated prompts across accepted, clarification, blocked, rejected, and response-too-large prediction-feature setup outcomes plus non-Helsinki setup-comprehension signals with approximate token counts and deterministic elapsed-time estimates.
...
The prediction-agent adoption generator writes a checked general adoption surface under `spec/fixtures/generated/prediction-agent-adoption/` and the root `ope.capabilities.json` manifest, exposing compact fit guidance, helps-with and does-not-provide lists, extension points, bring-your-own-model guidance, and a first-five-minutes adoption evaluation without adding runtime or quality claims. The prediction-goal catalog generator writes checked generic setup examples under `spec/fixtures/generated/prediction-goal-catalog/`, covering delivery delay, stockout, SLA breach, demand, churn, seaport berth, weather-sensitive operations, and public transit goal shapes without domain-quality claims. The setup-engine generator writes a checked canonical front door under `spec/fixtures/generated/setup-engine/`, turning a host prediction goal or structured setup request into candidate contracts, request readiness, source roles, baseline guidance, forecast-card preview shape, host-wrapper shape, catalog-backed examples, interface bindings, and claim boundaries before forecast artifacts exist.
