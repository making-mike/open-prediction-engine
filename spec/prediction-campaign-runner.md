...
Explicit local forecast creation is available with `--write-local`. The current execution slice performs a bounded foreground scheduling tick, derives the runner clock from real UTC now for effectful invocations (`--write-local`, `--watch`, `--live-weather`, `--execute-resolvers`) unless `--now` is supplied, re-derives that clock on every watch tick, writes standard lifecycle records plus minimal campaign/run state for the next due run under ignored `.ope/live/prediction-campaigns/`, and remains idempotent on repeat calls. Runs whose `forecastCloseAt` has already passed at launch are recorded as missed run states instead of backdated forecasts, and the write runtime independently refuses any forecast write after close regardless of how the clock was derived. With `--pre-calibrate`, the runner first computes a historical-only baseline pre-calibration binding and, when `--write-local` is also explicit, writes that binding before creating the ready forecast. With `--execute-resolvers`, the runner calls the checked campaign resolution-attempt readback for due runs. It also exposes the prospective method-binding path used after an approved `apply-method-update` command, but the dry-run default remains baseline-only and does not read ignored local method bindings. It does not fetch live data, create resolution records, create scoring records, append corpus evidence, or make quality claims. Long-running polling across later future windows remains a later slice.
...
python3 scripts/ope.py prediction-campaign start --pre-calibrate --write-local --output-format jsonl
```
...
- optional `--pre-calibrate` computes a historical-only baseline probability and writes a prospective local method binding only when paired with `--write-local`;
...
- without `--now`, effectful invocations clock against real UTC now (dry-run fixture readbacks keep the schedule-derived clock), so a live launch can never believe it is exactly on time for the first scheduled run;
...
- missed forecast close times are never backfilled: explicit local writes record a `missed` run state with `missed_forecast_close` as the exclusion reason, no forecast artifacts, and no comparable evidence append;
- captured stdout should use JSONL, while interactive terminals can print compact human status lines.

This contract is the bridge from campaign planning to a fuller foreground runner. It now implements normalized flag/setup-JSON campaign input, optional historical pre-calibration, a checked forecast scheduling plan, bounded foreground ticks with a real-UTC effectful clock, next-due local forecast creation, explicit missed-run state recording for windows past close, checked due-run resolver-attempt calls, and a schema-bound missed-run policy, while long-running future-window polling, effectful resolver execution, and calibration-ledger append remain later slices.
