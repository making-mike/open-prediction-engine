...
Explicit local execution is available with `--write-local`. It copies the checked lifecycle records into ignored local campaign state, writes minimal campaign/run state files with the idempotency key, refuses mismatched overwrites, and reports `already_present` on safe repeats. It does not fetch live data, run a resolver, create scores, append corpus evidence, or allow quality and calibration claims.
