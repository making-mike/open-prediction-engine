...
- the prediction-agent adoption surface now frames OPE as a prediction engine setup shortcut with built-in credibility gates, with checked `setup-engine` CLI, `agent-call`, local MCP readbacks, and catalog-backed generic examples as the canonical setup front door
- pilot evidence local writes are covered by the lifecycle operation store as receipt-backed `evidence.append` operations that update `pilot_findings` without calibration or track-record read-model effects
