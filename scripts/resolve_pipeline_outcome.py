#!/usr/bin/env python3
"""Resolve and score the local request-bound pipeline forecast."""
...
PIPELINE = ROOT / "spec" / "fixtures" / "generated" / "pipeline"
GENERATED = ROOT / "spec" / "fixtures" / "generated" / "pipeline-resolution"
...
PREFIX = "weather-logistics-pipeline-resolution"
RESOLVED_AT = "2026-06-04T10:40:00Z"
GENERATED_AT = "2026-06-04T10:45:00Z"
...
def pipeline_path(name: str) -> Path:
    return PIPELINE / f"weather-logistics-pipeline-{name}.generated.json"
...
def unscorable_resolution(
    ...
    return {
        "resolutionRecordId": "resolution-501",
...
def build_resolution(
    ...
    if operations.get("coverageStatus") != "complete":
        return unscorable_resolution(
            question,
            "annulled",
            "Operations outcome source does not cover the pipeline question geography and service date.",
            operations,
            weather_observation,
        )
    ...
    if weather_observation.get("sourceStatus") == "corrected":
        return unscorable_resolution(
            question,
            "stale_source",
            "Weather observation source was corrected after the pipeline forecast and requires review.",
            operations,
            weather_observation,
        )
    if observation.get("qualityFlag") != "complete":
        return unscorable_resolution(
            question,
            "ambiguous",
            "Weather observation source quality does not support a normal pipeline resolution.",
            operations,
            weather_observation,
        )
...
def build_reports(
    ...
    return {
        ...
        f"{PREFIX}-calibration.generated.json": {
            "calibrationSummaryId": "calibration-501",
            "generatedAt": GENERATED_AT,
            "domain": question["domain"],
        ...
        f"{PREFIX}-track-record.generated.json": {
            "trackRecordReportId": "trackrecord-501",
            "generatedAt": GENERATED_AT,
            "coveragePeriod": {
                "startsAt": question["openAt"],
                "endsAt": resolution["resolvedAt"],
...
def build_outputs() -> dict[str, Any]:
    pipeline_run = load_json(pipeline_path("pipeline-run"))
    question = load_json(pipeline_path("question"))
    feature_snapshot = load_json(pipeline_path("feature-snapshot"))
    evidence = load_json(pipeline_path("evidence"))
    artifact = load_json(pipeline_path("artifact"))
    history = load_json(pipeline_path("history"))
    ...
    outputs: dict[str, Any] = {
        ...
        f"{PREFIX}-outcome-summary.generated.json": {
            ...
            "qualityClaimStatus": "not_enough_resolved_pipeline_outcomes",
            "minimumCalibrationSampleSize": MIN_CALIBRATION_SAMPLE_SIZE,
            "resolvedComparablePipelineOutcomes": 1 if resolution["status"] == "resolved" else 0,
    ...
    validate_outputs(outputs, pipeline_run, question, evidence, artifact, history)
    return outputs
...
def validate_outputs(
    ...
    if pipeline_run["outputs"]["forecastId"] != evidence["forecastId"]:
        raise AssertionError("pipeline run/evidence forecast binding mismatch")
    ...
    if history["questionId"] != question["questionId"]:
        raise AssertionError("pipeline history/question binding mismatch")
    if resolution["questionId"] != question["questionId"]:
        raise AssertionError("pipeline resolution/question binding mismatch")
    if scoring["forecastId"] != evidence["forecastId"]:
        raise AssertionError("pipeline scoring/evidence forecast binding mismatch")
    if summary["requestId"] != pipeline_run["requestId"]:
        raise AssertionError("pipeline outcome summary/request binding mismatch")
    if resolution["status"] == "resolved" and scoring["scoreStatus"] != "scored":
        raise AssertionError("resolved pipeline outcome must be scored")
    if should_exclude_resolution(resolution) and scoring["scoreStatus"] != "excluded":
        raise AssertionError("unscorable pipeline outcome must be excluded")
    if summary["resolvedComparablePipelineOutcomes"] >= summary["minimumCalibrationSampleSize"]:
        raise AssertionError("pipeline fixture should remain below calibration threshold")
    ...
    if forecast_source_ids.intersection(resolution_source_ids):
        raise AssertionError("pipeline forecast evidence must not include future resolution sources")
...
def write_outputs(outputs: dict[str, Any]) -> None:
    ...
    print(f"generated {len(outputs)} pipeline resolution outputs")
...
def check_outputs(outputs: dict[str, Any]) -> None:
    ...
    for path in sorted(GENERATED.glob("*.generated.json")):
        if path.name not in expected_names:
            errors.append(f"stale pipeline resolution output: {path}")
    for filename, output in outputs.items():
        path = GENERATED / filename
        expected = render_json(output)
        if not path.exists():
            errors.append(f"missing pipeline resolution output: {path}")
            continue
        actual = path.read_text(encoding="utf-8")
        if actual != expected:
            errors.append(f"pipeline resolution drift: {path}")
    if errors:
        for error in errors:
            print(error, file=sys.stderr)
        print("run `python3 scripts/resolve_pipeline_outcome.py --write`", file=sys.stderr)
        raise SystemExit(1)
    print(f"checked {len(outputs)} pipeline resolution outputs")
...
def check_unscorable_variants() -> None:
    ...
    if build_resolution(question, feature_snapshot, missing_ops, weather_observation)["status"] != "annulled":
        raise AssertionError("missing operations coverage should annul the pipeline outcome")
...
def main() -> None:
    ...
    parser.add_argument("--write", action="store_true", help="write generated pipeline resolution outputs")
