...
def build_runbook_steps() -> list[dict[str, Any]]:
    return [
        ...
        runbook_step(
            ...
            operator_check="Run only inside the forecast creation window and keep stdout/logs; the launch clock is real UTC now, so --now is never needed for a live launch.",
            success_signal="One ready forecast is created idempotently in .ope/live for the next due run, with forecastedAt stamped at the actual launch time.",
            recovery_action="If close time has passed, the launch records the run as missed automatically and creates nothing; never backfill.",
