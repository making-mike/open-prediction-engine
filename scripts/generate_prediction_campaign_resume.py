...
def check_or_write(data: dict[str, Any], *, write: bool) -> None:
    ...
    if not OUTPUT_PATH.exists():
        raise SystemExit(f"Missing generated prediction campaign resume fixture: {OUTPUT_PATH}")
    existing = OUTPUT_PATH.read_text(encoding="utf-8")
    if existing != rendered:
        raise SystemExit("prediction campaign resume fixture drifted; run with --write")
