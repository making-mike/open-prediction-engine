...
For repo transfers, pair this skill with [transferable-agent-baseline.md](../../resources/transferable-agent-baseline.md). Keep the general agentic-economy assumptions, but replace project-specific protocol claims, well-known paths, commands, and non-goals.
...
> **Rule of thumb:** Single-agent loops can degrade materially over long, multi-step, or adversarial tasks. Switch to multi-agent coordination when measured failures justify the added complexity.
...
The **A2A protocol** (Linux Foundation A2A project, originated by Google) is the "HTTP of agents" — standardizing how agents from different vendors collaborate.

**Core element:** The **Agent Card** — a JSON metadata document advertising identity, endpoints, modalities, and auth requirements. Current A2A well-known discovery uses `/.well-known/agent-card.json` when the well-known URI strategy is used.

**Project overlay:** Treat external agent discovery as an optional integration surface. Do not hard-code project-specific discovery paths in general guidance, and do not claim full compatibility with a protocol unless the required task, message, transport, and security behavior is implemented.

| Component | Format | Function |
|---|---|---|
| Agent Card | JSON object / schema | Discovery & capability matching |
...
For production MCP servers, expose the smallest necessary set of tools/resources/prompts, return tool lists in deterministic order where possible, keep credentials out of prompt-visible arguments, and require host approval for non-read-only or paid tools.
